...
__global__ void Sgemm128x128Divisible128Bug(float *mat_a, float *mat_b, float *mat_c, int m, int k, int n)
{
    int tid = threadIdx.x;
    int warp_id = tid / 32;
    int lane_id = tid % 32;
    ...
    for (int i = 0; i < k; i += 8)
    {
        // load data(mat_a), gmem -> rf -> shm, transpose for consecutive access in outer product
        FETCH_FLOAT4(mat_a_frag[0]) =
            FETCH_FLOAT4(mat_a[OFFSET(i + tid_mod_2 * 4, start_row_c + tid_div_2, k)]);
        ...
        // load data(mat_b), gmem -> shm
        FETCH_FLOAT4(mat_b_shm[warp_id][lane_id * 4]) =
            FETCH_FLOAT4(mat_b[OFFSET(start_col_c + lane_id * 4, i + warp_id, n)]);

        __syncthreads();

        // threadblock-256, inter-warp(2x4), block tile: 128x128
        // warp0 warp2 warp4 warp6
        // warp1 warp3 warp5 warp7

        // inner-warp(8x4), thread tile(total: 8x8), warp tile: 64x32(32x16 in single step, 2x2 iteration)
        /*
        x x x x
        x x x x
        x x x x
...
#pragma unroll
        ...
        {
            // 4 element load(LDS.128) in single instruction, two instructions
            FETCH_FLOAT4(mat_a_frag[0]) = FETCH_FLOAT4(mat_a_shm[j][(warp_id % 2) * 64 + (tid % 4) * 4]);
            FETCH_FLOAT4(mat_a_frag[4]) = FETCH_FLOAT4(mat_a_shm[j][(warp_id % 2) * 64 + (tid % 4) * 4 + 32]);

            FETCH_FLOAT4(mat_b_frag[0]) = FETCH_FLOAT4(mat_b_shm[j][(warp_id / 2) * 32 + (tid % 8) * 4]);
            FETCH_FLOAT4(mat_b_frag[4]) = FETCH_FLOAT4(mat_b_shm[j][(warp_id / 2) * 32 + (tid % 8) * 4 + 16]);
...
#pragma unroll
    ...
    int write_base_offset = OFFSET(start_col_c, start_row_c, n) +               /* block offset */
                            OFFSET((warp_id / 2) * 32, (warp_id % 2) * 64, n) + /* warp offset */
                            OFFSET((tid / 8) * 4, tid % 8, n);                  /* thread offset */
// write to mat_c
#pragma unroll
    for (int i = 0; i < 4; ++i)
    {
        ...
        // o o
        // o x
        // if (write_base_offset + 32 * n + i * n + 16 == 512) {
        //     printf("C11, tid: %d,\n", tid);
        // }
        FETCH_FLOAT4(mat_c[write_base_offset + 32 * n + i * n + 16]) = FETCH_FLOAT4(mat_c_accum[i + 4][4]);
    }
...
// modify arg-m or arg-n, still pass, so bug operations in dim-k?
__global__ void Sgemm128x128BugK(float *mat_a, float *mat_b, float *mat_c, int m, int k, int n)
{
    int tid = threadIdx.x;
    int warp_id = tid / 32;
    int lane_id = tid % 32;
...
#pragma unroll
    ...
    for (int i = 0; i < k; i += 8)
    {
        ...
        if (ver_offset < m)
        {
            for (int j = hor_offset; j < k && j < hor_offset + 4; ++j)
            {
                mat_a_shm[tid_mod_2 * 4 +
                          j - hor_offset][tid_div_2] = mat_a[OFFSET(j, ver_offset, k)];
            }
        }
        ...
        if (ver_offset < k)
        {
            for (int j = hor_offset; j < n && j < hor_offset + 4; ++j)
            {
                mat_b_shm[warp_id][lane_id * 4 + j - hor_offset] = mat_b[OFFSET(j, ver_offset, n)];
            }
        }
...
__global__ void Sgemm128x128Bug(float *mat_a, float *mat_b, float *mat_c, int m, int k, int n)
{
    int tid = threadIdx.x;
    int warp_id = tid / 32;
    int lane_id = tid % 32;
...
#pragma unroll
    ...
    for (int i = 0; i < k; i += 8)
    {
        ...
        if (ver_offset < m && hor_offset < k)
        {
            if (hor_offset < k - 3)
            { // last one, {k-4, k-3, k-2, k-1}
                FETCH_FLOAT4(mat_a_frag[0]) = FETCH_FLOAT4(mat_a[OFFSET(hor_offset, ver_offset, k)]);
            }
            else
            {
                for (int j = hor_offset; j < k; ++j)
                { // 0[, 1[, 2]]
                    mat_a_frag[j - hor_offset] = mat_a[OFFSET(j, ver_offset, k)];
                }
            }
        }
        ...
        if (ver_offset < k && hor_offset < n)
        {
            if (hor_offset < n - 3)
            {
                FETCH_FLOAT4(mat_b_shm[warp_id][lane_id * 4]) =
                    FETCH_FLOAT4(mat_b[OFFSET(hor_offset, ver_offset, n)]);
            }
            else
            {
                for (int j = hor_offset; j < n; ++j)
                {
                    mat_b_shm[warp_id][lane_id * 4 + j - hor_offset] = mat_b[OFFSET(j, ver_offset, n)];
                }
            }
...
#pragma unroll
    ...
    int hor_offset_c = start_row_c + (warp_id % 2) * 64 + (lane_id % 8) * 4;
    int ver_offset_c = start_col_c + (warp_id / 2) * 32 + (lane_id / 8) * 4;
// write to mat_c
#pragma unroll
    for (int i = 0; i < 4; ++i)
    {
        // x o
        // o o
        if (ver_offset_c + i < m && hor_offset_c < n)
        {
            if (hor_offset_c < n - 3)
            {
                FETCH_FLOAT4(mat_c[write_base_offset + i * n]) = FETCH_FLOAT4(mat_c_accum[i][0]);
            ...
            {
                for (int j = hor_offset_c; j < n; ++j)
                {
                    mat_c[write_base_offset + i * n +
                          j - hor_offset_c /* 0[, 1[, 2]] */] = mat_c_accum[i][j - hor_offset_c];
                }
            }
        }
    }
...
#pragma unroll
    for (int i = 0; i < 4; ++i)
    {
        ...
        if (ver_offset_c + i < m && hor_offset_c < n)
        {
            if (hor_offset_c < n - 3)
            {
                FETCH_FLOAT4(mat_c[write_base_offset + i * n]) = FETCH_FLOAT4(mat_c_accum[i][4]);
            }
            else
            {
                for (int j = hor_offset_c; j < n; ++j)
                {
                    mat_c[write_base_offset + i * n +
                          j - hor_offset_c /* 0[, 1[, 2]] */] = mat_c_accum[i][4 + j - hor_offset_c];
                }
            }
        }
    }
...
#pragma unroll
    for (int i = 0; i < 4; ++i)
    {
        ...
        if (ver_offset_c + i < m && hor_offset_c < n)
        {
            if (hor_offset_c < n - 3)
            {
                FETCH_FLOAT4(mat_c[write_base_offset + i * n]) = FETCH_FLOAT4(mat_c_accum[i + 4][0]);
            }
            else
            {
                for (int j = hor_offset_c; j < n; ++j)
...
#pragma unroll
    for (int i = 0; i < 4; ++i)
    {
        ...
        if (ver_offset_c + i < m && hor_offset_c < n)
        {
            if (hor_offset_c < n - 3)
            {
                FETCH_FLOAT4(mat_c[write_base_offset + i * n]) = FETCH_FLOAT4(mat_c_accum[i + 4][4]);
            }
            else
            {
                for (int j = hor_offset_c; j < n; ++j)
                {
                    mat_c[write_base_offset + i * n +
                          j - hor_offset_c /* 0[, 1[, 2]] */] = mat_c_accum[i + 4][4 + j - hor_offset_c];
                }
            }
        }
    }
...
// some small opt for kernel "Sgemm128x128Buf2"
__global__ void Sgemm128x128Buf2Opt_back(float *mat_a, float *mat_b, float *mat_c, int m, int k, int n)
{
    int tid = threadIdx.x;
    int warp_id = tid / 32;
    int lane_id = tid % 32;
    ...
    int a_shm_offset = (warp_id / 2) * 32 + (lane_id / 8) * 4;
    int b_shm_offset = (warp_id % 2) * 64 + (lane_id % 8) * 4;
    ...
    // rf produce: load first rf buffer(step 1 of 8), smem -> rf
    // warp shape: 4x8
    {
        FETCH_FLOAT4(mat_a_frag[0][0]) =
            FETCH_FLOAT4(mat_a_shm[0][0][a_shm_offset]);
        FETCH_FLOAT4(mat_a_frag[0][4]) =
            FETCH_FLOAT4(mat_a_shm[0][0][a_shm_offset + 16]);

        FETCH_FLOAT4(mat_b_frag[0][0]) =
            FETCH_FLOAT4(mat_b_shm[0][0][b_shm_offset]);
        FETCH_FLOAT4(mat_b_frag[0][4]) =
            FETCH_FLOAT4(mat_b_shm[0][0][b_shm_offset + 32]);
    }
    ...
    for (int i = 8; i < k; i += 8)
    {
        // load next mat_a & mat_b tile, gmem -> rf
        {
            ver_offset = start_row_c + tid_div_2; // vertical offset
            hor_offset = i + tid_mod_2 * 4;       // horizontal offset
...
#pragma unroll
        for (int j = 0; j < 8; ++j)
        {
            ...
            // 4 element load(LDS.128) in single instruction, two instructions
            FETCH_FLOAT4(mat_a_frag[(j + 1) % 2][0]) =
                FETCH_FLOAT4(mat_a_shm[smem_consume_id][(j + 1) % 8][a_shm_offset]);
            FETCH_FLOAT4(mat_a_frag[(j + 1) % 2][4]) =
                FETCH_FLOAT4(mat_a_shm[smem_consume_id][(j + 1) % 8][a_shm_offset + 16]);

            FETCH_FLOAT4(mat_b_frag[(j + 1) % 2][0]) =
                FETCH_FLOAT4(mat_b_shm[smem_consume_id][(j + 1) % 8][b_shm_offset]);
            FETCH_FLOAT4(mat_b_frag[(j + 1) % 2][4]) =
                FETCH_FLOAT4(mat_b_shm[smem_consume_id][(j + 1) % 8][b_shm_offset + 32]);
...
#pragma unroll
    ...
    int ver_offset_c = start_row_c + a_shm_offset;
    int hor_offset_c = start_col_c + b_shm_offset;
// write to mat_c
#pragma unroll
    for (int i = 0; i < 4; ++i)
    {
