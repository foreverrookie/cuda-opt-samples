...
#define PROFILE_SGEMM(expr, name, m, k, n)                                  \
    do {                                                                    \
...
void CheckCudaOutput(const std::vector<float> &cublas_output, float *mat_c_dev) {
    int mn = cublas_output.size();
    std::vector<float> cuda_output(mn);

    CUDA_CHECK(
        cudaMemcpy(cuda_output.data(), mat_c_dev, mn * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaGetLastError());

    for (int i = 0; i < mn; ++i) {
        // compare relative error
        if (std::fabs((cublas_output[i] - cuda_output[i]) / cublas_output[i]) > 1e-5) {
            std::cout << "index of error value: " << i << ", cuBLAS value: " << cublas_output[i]
                      << ", my value: " << cuda_output[i] << std::endl;
            throw std::runtime_error("compute error!");
        }
    }

    std::cout << "Check Output Status: Pass! \n";
    std::cout << std::endl;
}

void CublasSgemm(float *mat_a, float *mat_b, float *mat_c, std::vector<float> &cublas_c, int m,
                 int k, int n) {
    ...
    // warm up
    CUBLAS_CHECK(cublasSgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha, mat_b, n,
                             mat_a, k, &beta, mat_c, n));

    // NT, T - transpose, n - no transpose
    // C = A*B, A B C stored by column-major
    // if row-major output wanted, convert to CT = BT * AT
    PROFILE_SGEMM(CUBLAS_CHECK(cublasSgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha,
                                           mat_b, n, mat_a, k, &beta, mat_c, n)),
                  cublasSgemm, m, k, n);

    CUDA_CHECK(cudaMemcpy(cublas_c.data(), mat_c, m * n * sizeof(float), cudaMemcpyDeviceToHost));
...
__global__ void Sgemm128x128Divisible128(float *mat_a, float *mat_b, float *mat_c, int m, int k,
                                         int n) {
    int tid = threadIdx.x;
    int warp_id = tid / 32;
    int lane_id = tid % 32;
...
#pragma unroll
        for (int j = 0; j < 8; ++j) {
            ...
            FETCH_FLOAT4(mat_a_frag[0]) =
                FETCH_FLOAT4(mat_a_shm[j][(warp_id % 2) * 64 + (lane_id % 8) * 4]);
            FETCH_FLOAT4(mat_a_frag[4]) =
                FETCH_FLOAT4(mat_a_shm[j][(warp_id % 2) * 64 + (lane_id % 8) * 4 + 32]);

            FETCH_FLOAT4(mat_b_frag[0]) =
                FETCH_FLOAT4(mat_b_shm[j][(warp_id / 2) * 32 + (lane_id / 8) * 4]);
            FETCH_FLOAT4(mat_b_frag[4]) =
                FETCH_FLOAT4(mat_b_shm[j][(warp_id / 2) * 32 + (lane_id / 8) * 4 + 16]);
...
#pragma unroll
    for (int i = 0; i < 4; ++i) {
        // o o
        // x o
        FETCH_FLOAT4(mat_c[write_base_offset + 32 * n + i * n]) =
            FETCH_FLOAT4(mat_c_accum[i + 4][0]);
    }
...
#pragma unroll
    for (int i = 0; i < 4; ++i) {
        // o o
        // o x
        FETCH_FLOAT4(mat_c[write_base_offset + 32 * n + i * n + 16]) =
            FETCH_FLOAT4(mat_c_accum[i + 4][4]);
    }
}
...
#pragma unroll
    ...
    for (int i = 0; i < k; i += 8) {
        ...
        if (ver_offset < m) {
            for (int j = 0; j < 4; ++j) {
                if (j + hor_offset < k) {
                    mat_a_shm[tid_mod_2 * 4 + j][tid_div_2] =
                        mat_a[OFFSET(j + hor_offset, ver_offset, k)];
                } else {
        ...
        if (ver_offset < k) {
            for (int j = 0; j < 4; ++j) {
                if (j + hor_offset < n) {
                    mat_b_shm[warp_id][lane_id * 4 + j] =
                        mat_b[OFFSET(j + hor_offset, ver_offset, n)];
                } else {
...
#pragma unroll
    for (int i = 0; i < 4; ++i) {
        ...
        if (ver_offset_c + 32 + i < m) {
            for (int j = 0; (j < 4) && (j < n - hor_offset_c - 16); ++j) {
                mat_c[OFFSET(hor_offset_c + j + 16, ver_offset_c + 32 + i, n)] =
                    mat_c_accum[i + 4][4 + j];
            }
        }
    }
...
__global__ void Sgemm128x128Buf2Divisible128(float *mat_a, float *mat_b, float *mat_c, int m, int k,
                                             int n) {
    int tid = threadIdx.x;
    int warp_id = tid / 32;
    int lane_id = tid % 32;
...
#pragma unroll
        for (int j = 0; j < 8; ++j) {
            ...
            // 4 element load(LDS.128) in single instruction, two instructions
            FETCH_FLOAT4(mat_a_frag[(j + 1) % 2][0]) = FETCH_FLOAT4(
                mat_a_shm[smem_consume_id][(j + 1) % 8][(warp_id % 2) * 64 + (lane_id % 8) * 4]);
            FETCH_FLOAT4(mat_a_frag[(j + 1) % 2][4]) =
                FETCH_FLOAT4(mat_a_shm[smem_consume_id][(j + 1) % 8]
                                      [(warp_id % 2) * 64 + (lane_id % 8) * 4 + 32]);

            FETCH_FLOAT4(mat_b_frag[(j + 1) % 2][0]) = FETCH_FLOAT4(
                mat_b_shm[smem_consume_id][(j + 1) % 8][(warp_id / 2) * 32 + (lane_id / 8) * 4]);
            FETCH_FLOAT4(mat_b_frag[(j + 1) % 2][4]) =
                FETCH_FLOAT4(mat_b_shm[smem_consume_id][(j + 1) % 8]
                                      [(warp_id / 2) * 32 + (lane_id / 8) * 4 + 16]);
...
__global__ void Sgemm128x128Buf2(float *mat_a, float *mat_b, float *mat_c, int m, int k, int n) {
    ...
    // produce: load first smem buffer(step 1 of 2), gmem -> smem
    {
        ...
        if (ver_offset < m) {
            for (int j = 0; j < 4; ++j) {
                if (j + hor_offset < k) {
                    mat_a_shm[0][tid_mod_2 * 4 + j][tid_div_2] =
                        mat_a[OFFSET(j + hor_offset, ver_offset, k)];
                } else {
        ...
        if (ver_offset < k) {
            for (int j = 0; j < 4; ++j) {
                if (j + hor_offset < n) {
                    mat_b_shm[0][warp_id][lane_id * 4 + j] =
                        mat_b[OFFSET(j + hor_offset, ver_offset, n)];
                } else {
...
__global__ void Sgemm128x128Buf2OptDivisible128(float *mat_a, float *mat_b, float *mat_c, int m,
                                                int k, int n) {
    int tid = threadIdx.x;
    int warp_id = tid / 32;
    int lane_id = tid % 32;
    ...
    __shared__ float mat_a_shm[2][8][132];
    __shared__ float mat_b_shm[2][8][128];  // 8 x 128, double buffer

    float mat_a_ldg128[4];  // load global(LDG.128)
    float mat_b_ldg128[4];  // load global(LDG.128)
    ...
    // produce: load first smem buffer(step 1 of 2), gmem -> smem
    // load mat_a(LDG.128), gmem -> rf -> shm
    {
        FETCH_FLOAT4(mat_a_ldg128[0]) =
            FETCH_FLOAT4(mat_a[OFFSET(0, start_row_c, k) +          /* main loop offset */
                               OFFSET((tid % 2) * 4, tid / 2, k)]); /* thread offset */

        // transpose for consecutive access in outer product computation phase
        mat_a_shm[0][(tid % 2) * 4][tid / 2] = mat_a_ldg128[0];
        mat_a_shm[0][(tid % 2) * 4 + 1][tid / 2] = mat_a_ldg128[1];
        mat_a_shm[0][(tid % 2) * 4 + 2][tid / 2] = mat_a_ldg128[2];
        mat_a_shm[0][(tid % 2) * 4 + 3][tid / 2] = mat_a_ldg128[3];
    ...
    // produce: load first rf buffer(step 1 of 8), smem -> rf
    {
        FETCH_FLOAT4(mat_a_frag[0][0]) = FETCH_FLOAT4(mat_a_shm[0][0][shm_a_offset]);
        FETCH_FLOAT4(mat_a_frag[0][4]) = FETCH_FLOAT4(mat_a_shm[0][0][shm_a_offset + 16]);

        FETCH_FLOAT4(mat_b_frag[0][0]) = FETCH_FLOAT4(mat_b_shm[0][0][shm_b_offset]);
        FETCH_FLOAT4(mat_b_frag[0][4]) = FETCH_FLOAT4(mat_b_shm[0][0][shm_b_offset + 32]);
    }
...
#pragma unroll
        for (int j = 0; j < 8; ++j) {
            // smem produce: store next mat_a & mat_b tile, rf -> smem
            if (j == 7) {
                mat_a_shm[smem_consume_id ^ 1][(tid % 2) * 4][tid / 2] = mat_a_ldg128[0];
                mat_a_shm[smem_consume_id ^ 1][(tid % 2) * 4 + 1][tid / 2] = mat_a_ldg128[1];
                mat_a_shm[smem_consume_id ^ 1][(tid % 2) * 4 + 2][tid / 2] = mat_a_ldg128[2];
                mat_a_shm[smem_consume_id ^ 1][(tid % 2) * 4 + 3][tid / 2] = mat_a_ldg128[3];
...
#pragma unroll
    for (int i = 0; i < 4; ++i) {
        // o o
        // x o
        FETCH_FLOAT4(mat_c[write_base_offset + 16 * n + i * n]) =
            FETCH_FLOAT4(mat_c_accum[i + 4][0]);
    }
...
#pragma unroll
    for (int i = 0; i < 4; ++i) {
        // o o
        // o x
        FETCH_FLOAT4(mat_c[write_base_offset + 16 * n + i * n + 32]) =
            FETCH_FLOAT4(mat_c_accum[i + 4][4]);
    }
}
...
<<<<<<< HEAD
    ...
    == == == =
                 __shared__ float __align__(16) mat_b_shm[2][8][128];  // 8 x 128, double buffer
>>>>>>> 9d61c7c (feat: add reduce op & some opt in sgemm)
    ...
    // produce: load first smem buffer(step 1 of 2), gmem -> smem
    {
        // load mat_a(LDG.128), gmem -> rf -> shm
        if (ver_offset < m) {
            mat_a_shm[0][tid_mod_2 * 4][tid_div_2] = 0.0f;
            mat_a_shm[0][tid_mod_2 * 4 + 1][tid_div_2] = 0.0f;
            mat_a_shm[0][tid_mod_2 * 4 + 2][tid_div_2] = 0.0f;
            mat_a_shm[0][tid_mod_2 * 4 + 3][tid_div_2] = 0.0f;
...
#pragma unroll
            for (int j = 0; (j < 4) && (j < n - hor_offset_c - 32); ++j) {
                mat_c[OFFSET(hor_offset_c + j + 32, ver_offset_c + 16 + i, n)] =
                    mat_c_accum[i + 4][4 + j];
            }
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    CUDA_CHECK(cudaMemcpy(mat_a_dev, &mat_a[0], m * k * sizeof(float), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(mat_b_dev, &mat_b[0], k * n * sizeof(float), cudaMemcpyHostToDevice));
    ...
    dim3 grid1(DivUp(n, 16), DivUp(m, 16));
    {
        ...
        PROFILE_SGEMM((SgemmNaive<<<grid1, block1>>>(mat_a_dev, mat_b_dev, mat_c_dev, m, k, n)),
                      sgemm_naive, m, k, n);
        CheckCudaOutput(cublas_c, mat_c_dev);
    }
    if (k % 2 == 0) {
        // sgemm_naive2 time: 60.8432 ms,  FP32 Perf: 2.25891 TFlops
        PROFILE_SGEMM((SgemmNaive2<<<grid1, block1>>>(mat_a_dev, mat_b_dev, mat_c_dev, m, k, n)),
                      sgemm_naive2, m, k, n);
        CheckCudaOutput(cublas_c, mat_c_dev);
    }
    if (k % 4 == 0) {
        // sgemm_naive4 time: 54.8171 ms,  FP32 Perf: 2.50723 TFlops
        PROFILE_SGEMM((SgemmNaive4<<<grid1, block1>>>(mat_a_dev, mat_b_dev, mat_c_dev, m, k, n)),
                      sgemm_naive4, m, k, n);
        CheckCudaOutput(cublas_c, mat_c_dev);
    }

    {
        ...
        PROFILE_SGEMM((Sgemm16x16<<<grid1, block1>>>(mat_a_dev, mat_b_dev, mat_c_dev, m, k, n)),
                      sgemm_16x16, m, k, n);
        CheckCudaOutput(cublas_c, mat_c_dev);
    }
    ...
    dim3 grid2(DivUp(n, 128), DivUp(m, 128));
    {
        ...
        PROFILE_SGEMM((Sgemm128x128<<<grid2, block2>>>(mat_a_dev, mat_b_dev, mat_c_dev, m, k, n)),
                      sgemm_128x128, m, k, n);
        CheckCudaOutput(cublas_c, mat_c_dev);
    }
    {
        ...
        PROFILE_SGEMM((Sgemm128x128Buf2<<<grid2, block2>>>(mat_a_dev, mat_b_dev, mat_c_dev,
                                                           m, k, n)),
    ...
    {
        // sgemm_128x128_2buf_opt_Divisible128 time: 6.01779 ms,     FP32 Perf: 22.8388 TFlops
        // sgemm_128x128_2buf_opt              time: 7.77392 ms,     FP32 Perf: 17.6795 TFlops
        PROFILE_SGEMM((Sgemm128x128Buf2Opt<<<grid2, block2>>>(mat_a_dev, mat_b_dev,
                                                              mat_c_dev, m, k, n)),
